...
namespace QDP {
  ...
  template<class T>
  struct InternalScalar<WordVec<T> > {
  ...
  struct UnaryReturn<WordVec<T>, FnLocalNorm2 > {
    typedef WordVec<typename UnaryReturn<T, FnLocalNorm2>::Type_t>  Type_t;
  };

  
  // trying
  template<class T1, class Op>
  struct UnaryReturn<WordVec<T1>, Op> {
    typedef WordVec<typename UnaryReturn<T1, Op>::Type_t>  Type_t;
  };

  template<class T1, class T2, class Op>
  struct BinaryReturn<Word<T1>, WordVec<T2>, Op> {
    typedef WordVec<typename BinaryReturn<T1, T2, Op>::Type_t>  Type_t;
  };

  template<class T1, class T2, class Op>
  struct BinaryReturn<WordVec<T1>, Word<T2>, Op> {
    typedef WordVec<typename BinaryReturn<T1, T2, Op>::Type_t>  Type_t;
  };
  // end trying
  ...
  template<class T>
  struct UnaryReturn<WordVec<T>, OpNot > {
    typedef WordVec<typename UnaryReturn<T, OpNot>::Type_t>  Type_t;
  };

  template<class T1>
  inline typename UnaryReturn<WordVec<T1>, OpNot>::Type_t
  operator!(const WordVec<T1>& l)
  {
    return ! l.elem();
  }
#endif

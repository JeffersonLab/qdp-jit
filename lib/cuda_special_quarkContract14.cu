...
__device__ void quarkContractXX_14( int i , int j ,
				    cuComplex (&A)[4][4][3][3],
				    cuComplex (&B)[4][4][3][3],
				    cuComplex (&C)[4][4][3][3] )
{
...
__device__ void quarkContractXX_14_accum( int i , int j , int k ,
					  cuComplex (&A)[4][4][3][3],
					  cuComplex (&B)[4][4][3][3],
					  cuComplex (&C)[4][4][3][3] )
{
...
__global__ void quarkContract14_l( int N , float* A, float* B, float* C)
{
  ...
  for(int j=0; j < 4; ++j)
    for(int i=0; i < 4; ++i)
      {
	quarkContractXX_14( i , j , Al, Bl, Cl);

	for(int k=1; k < 4; ++k)
	  quarkContractXX_14_accum( i , j , k , Al , Bl , Cl );
      }
...
void evaluate_special_quarkContract14( int N, std::vector<void*> args )
{
  ...
  quarkContract14_l<<< grid , block >>>( N , (float*)args[0] , (float*)args[1] , (float*)args[2] );
}
